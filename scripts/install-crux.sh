#!/usr/bin/env bash
#
# install-crux.sh — Instalación de CRUX + gestor de paquetes elun.
...
REPO="${REPO:-https://github.com/pansususu/elun}"
DISK="${DISK:-}"
...
if ! chrun "command -v git >/dev/null 2>&1"; then
    ...
    chrun "export PATH=/root/.cargo/bin:\$PATH; \\
           git clone --depth 1 $REPO /usr/local/src/elun 2>/dev/null || true; \\
           cd /usr/local/src/elun && \\
           cargo build --release && \\
           install -Dm755 target/release/elun /usr/local/bin/elun"
fi
# ELUN_DIR para que las recetas se resuelvan desde el árbol clonado.
mkdir -p "$MOUNT/usr/local/src/elun/recipes" "$MOUNT/usr/local/src/elun/firecipes"
mkdir -p "$MOUNT/etc/profile.d"
cat > "$MOUNT/etc/profile.d/zz-elun.sh" <<'EOF'
ELUN_DIR=/usr/local/src/elun
export ELUN_DIR
EOF
...
echo "Luego loguea como $USERNAME y ejecuta 'elun' para verificar."
echo "Nota: el gestor baja releases de $REPO; ajusta ELUN_PREFIX/ELUN_DIR si instalas en otro sitio."
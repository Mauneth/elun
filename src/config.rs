...
static PREFIX: LazyLock<PathBuf> = LazyLock::new(|| match std::env::var("ELUN_PREFIX") {
    Ok(p) if !p.is_empty() => PathBuf::from(p),
    _ => PathBuf::from("/usr/local"),
});

/// Directorio base para recetas y firecipes. Defecto: prefijo/share/elun.
/// Sobrescribible con `ELUN_DIR` (dev, o donde instalaste el árbol de
/// recetas, p.ej. /usr/local/src/elun).
pub fn base_dir() -> PathBuf {
    BASE_DIR.clone()
}

static BASE_DIR: LazyLock<PathBuf> = LazyLock::new(|| match std::env::var("ELUN_DIR") {
    Ok(p) if !p.is_empty() => PathBuf::from(p),
    _ => prefix().join("share/elun"),
});
...
/// monte su servidor (`ELUN_RELEASES` / `ELUN_RELEASES`).
pub fn releases_repo() -> String {
    RELEASES_REPO.clone()
}

static RELEASES_REPO: LazyLock<String> =
    LazyLock::new(|| match std::env::var("ELUN_RELEASES") {
